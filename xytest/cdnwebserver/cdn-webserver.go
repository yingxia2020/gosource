...
func xgb_StatusHandler(ctx *fasthttp.RequestCtx) {
	//fmt.Fprintf(ctx, "cdn-weberver xgb_StatusHandler\n")
	if testinProgress == true {
		...
		//log.Printf("resultBuf1:%s\n", resultBuf )
		fmt.Fprintf(ctx, "Test is Done:\n")
		fmt.Fprintf(ctx, "%s\n", resultBuf.String())
		//var folderName string = "/mnt/data/" + resultsFolder + "/"
		//var folderName string = "./output/" + resultsFolder + "/"
		//if _, err := os.Stat(folderName); os.IsNotExist(err) {
		//	os.Mkdir(folderName, os.FileMode(0577))
		//}
		//f, err := os.Create(folderName + "/" + resultsFolder + ".csv")
		//if err != nil {
		//	fmt.Println(err)
		//	return
		//}
		//defer f.Close()
		//f.WriteString(resultBuf.String())
		//f.Sync()
	}
...
func xgb_StartTestHandler(ctx *fasthttp.RequestCtx) {
	...
	//go startKafkaProducer(numberOfMessages)
	...
	go func() {
		...
		for {
			...
			case msg := <-consumer.Messages():
				...
				//resultBuf.WriteString(tr.Key + ":" + tr.Duration + ", ")
				//log.Printf("resultBuf2:%S", resultBuf.String())
				//Time to process the request
				processTime := time.Now()
				timeProcess := processTime.Sub(processStartTime).Seconds()
				processDuration := strconv.FormatFloat(timeProcess, 'f', 2, 64)
				//log.Println(tr.Key + ":" + tr.Duration + " - " + processDuration + ", ")
				log.Println(tr.Key + ":" + processDuration + ", ")

				//ToDo - extract queue_duration from tr.Key
				...
				//transactiontime := processTime.Sub(message_startqueue)
				...
				//log.Println(tr.Key + ":" + tr.Duration + ", ")
				...
				if msgCount == numMessages {
					...
					//resultBuf.WriteString("TotalDuration:"+ processTotalDuration + ", ")
...
func startKafkaProducer_PoissonGenerator(lambda float64, numMessages int) {
	...
	go func() {
		...
		//lambda := 9.00
		//numMessages := 100
		seed := int64(1582221789833178776) // time.Now().UnixNano()
		...
		for i := 0; i < numMessages+2; i++ {
			...
			case err := <-producer.Errors():
				...
				//log.Fatalf("cmd.Run() failed with CoronaVirus\n")
				//os.Exit(2)
				panic(err)
				break
	...
	// ========================================================
	//generate n messages every Second
	...
	// 	log.Printf("Go funcPoisson dataset:%s", dataset)
	//
	// 	time_counter_key:=int64(0)
	// 	for _, key := range keys {
	// 		fmt.Printf("%d\t%d\n", key, hist[key])
	//
	// 		if (time_counter_key < key){
	// 			for { //catch up to the propper time
	// 				log.Printf(" TIME %d NO message generated... %d mges delivered \n",time_counter_key, msg_id_enqueued)
	// 				time_counter_key++
	// 				time.Sleep( 1 * time.Minute)
	// 				if time_counter_key == key {
	// 					break
	...
	//
	// 		if (time_counter_key == key){
	// 			log.Printf(" TIME %d - Generate %d Messages\n",time_counter_key, hist[key])
	//
	// 			for transaction := 0; transaction < hist[key]; transaction++ {
	...
	go func() {
		...
		for {
			...
			msg := &sarama.ProducerMessage{
				Topic: "ktopic-xgboost",
				Key:   sarama.ByteEncoder(strconv.Itoa(msg_id_enqueued)),
				//Partition: int32(msg_id_enqueued),
